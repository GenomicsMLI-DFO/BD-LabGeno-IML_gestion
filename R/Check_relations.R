...
# EXTRAIT : 2 checks
  ...
  if(c("Externe_librairies_WGS") %in%  names(data)  ){
    ...
    cat("\n", length(Numero_unique_librairie), "Numero_unique_librairie_WGS detected\n")
  ...
  if(c("WGS") %in%  names(data)  ){
    cat("\nImporting", crayon::cyan("WGS"), "table KEY:\n")

      # Check with specimens

    if(!is.null(Numero_unique_specimen)){

      if(all(data$WGS$Numero_unique_specimen %in% Numero_unique_specimen)){
        cat(crayon::green("\nAll the Numero_unique_specimen observed exist in the Specimen table.\n"))

      } else {

        cat(crayon::red("\nNot all the Numero_unique_specimen observed exist in the Specimen table.\n",
                        paste(unique(data$WGS$Numero_unique_specimen[!data$WGS$Numero_unique_specimen %in% Numero_unique_specimen]), collapse = ", ")
                        , "\nare missing\n"))
      }
    ...
    if(!is.null(Numero_unique_extrait)){
      ...
      if(all(data$WGS$Numero_unique_extrait %in% Numero_unique_extrait)){
        cat(crayon::green("\nAll the Numero_unique_extrait observed exist in the Extrait_ADN_ARN table.\n"))

      } else {

        cat(crayon::red("\nNot all the Numero_unique_extrait observed exist in the Extrait_ADN_ARN table.\n",
                        paste(unique(data$WGS$Numero_unique_extrait[!data$WGS$Numero_unique_extrait %in% Numero_unique_extrait]), collapse = ", ")
                        , "\nare missing\n"))
      }
     ...
     # Check with Numero_unique_pool_WGS
...
check_relation_ADNe  <- function(data){
    ...
    if(!is.null(Numero_unique_filtre)){
      ...
      if(all(data$Extraits_ADNe$Numero_unique_filtre_ADNe %in% Numero_unique_filtre)){
        ...
        cat(crayon::red("\nNot all the Numero_unique_filtre_ADNe observed exist in the Filtre_ADNe table.\n",
                        paste(unique(data$Extraits_ADNe$Numero_unique_filtre_ADNe[!data$Extraits_ADNe$Numero_unique_filtre_ADNe %in% Numero_unique_filtre]), collapse = ", "),
  ...
  # Librairie SeqReady :  2 checks

  if(c("Librairies_SeqReady_ADNe") %in%  names(data)  ){
    cat("\nImporting", crayon::cyan("Librairies_SeqReady_ADNe"), "table KEY:\n")

    # Check with Numero_unique_librairie_ADNe

    if(!is.null(Purif_ID)){

      if(all(data$Librairies_SeqReady_ADNe$Numero_unique_librairie_ADNe %in%  Purif_ID)){
        cat(crayon::green("\nAll the Numero_unique_librairie_ADNe observed exist in the Purification_librairies_ADNe table.\n"))
        ...
        cat(crayon::red("\nNot all the Numero_unique_librairie_ADNe observed exist in the Purification_librairies_ADNe table.\n",
                        ...
                        paste(unique(data$Librairies_SeqReady_ADNe$Numero_unique_librairie_ADNe[!data$Librairies_SeqReady_ADNe$Numero_unique_librairie_ADNe %in% Purif_ID]), collapse = ", ")
                        , "\nare missing\n"))
      }
  ...
  if(!is.null(Ext_ID)){
    ...
    if(all(data$Librairies_SeqReady_ADNe$Numero_unique_librairie_SeqReady_ADNe %in%  Ext_ID)){
      cat(crayon::green("\nAll the Numero_unique_librairie_SeqReady_ADNe observed exist in the Analyses_externes_librairies_ADNe table.\n"))
      ...
      cat(crayon::red("\nNot all the Numero_unique_librairie_SeqReady_ADNe observed exist in the Analyses_externes_librairies_ADNe table.\n",
                      ...
                      paste(unique(data$Librairies_SeqReady_ADNe$Numero_unique_librairie_SeqReady_ADNe[!data$Librairies_SeqReady_ADNe$Numero_unique_librairie_SeqReady_ADNe %in% Ext_ID]), collapse = ", ")
                      , "\nare missing\n"))
    }
...
check_relation_ADNe_wDB  <- function(data, DB = "LabGeno"){
  ...
  site.key <- load_columns_DB(columns = "Numero_unique_site_ADNe", table = "21_Sites_ADNe", DB = DB, verbose = F) |> dplyr::pull(Numero_unique_site_ADNe)
    ...
    # Check with site
      ...
      if(all(data$Stations_ADNe$Numero_unique_site_ADNe %in% c(Numero_unique_site, site.key))){
        ...
        cat(crayon::red("\nNot all the Numero_unique_site_ADNe observed exist in the Sites_ADNe table or DB.\n",
                        ...
                        paste(unique(data$Stations_ADNe$Numero_unique_site_ADNe[!data$Stations_ADNe$Numero_unique_site_ADNe %in% c(Numero_unique_site, site.key)]), collapse = ", ")
                        , "\nare missing\n"))
      }
  ...
  if(c("Echantillons_ADNe") %in%  names(data)  ){
    ...
    # Check with stations
      ...
      if(all(data$Echantillons_ADNe$Numero_unique_station_ADNe %in% c(Numero_unique_station, station.key))){
        ...
        cat(crayon::red("\nNot all the Numero_unique_station_ADNe observed exist in the Stations_ADNe table or DB.\n",


                        paste(unique(data$Echantillon_ADNe$Numero_unique_station_ADNe[!data$Echantillon_ADNe$Numero_unique_station_ADNe %in% c(Numero_unique_station, station.key)]), collapse = ", ")
                        , "\nare missing\n"))
      }
 ...
 #   }
  ...
  #  if(!is.null(Numero_unique_echantillon)){
      ...
      if(all(data$Filtres_ADNe$Numero_unique_echantillon_ADNe %in% c(Numero_unique_echantillon, echantillon.key) )){
        ...
        cat(crayon::red("\nNot all the Numero_unique_echantillon_ADNe observed exist in the Site_ADNe table or DB.\n",


                        paste(unique(data$Filtres_ADNe$Numero_unique_echantillon_ADNe[!data$Filtres_ADNe$Numero_unique_echantillon_ADNe %in% c(Numero_unique_echantillon, echantillon.key)]), collapse = ", ")
                        , "\nare missing\n"))
      }
  ...
  if(c("Extraits_ADNe") %in%  names(data)  ){
    ...
    # Check with filtre
      ...
      if(all(data$Extraits_ADNe$Numero_unique_filtre_ADNe %in% c(Numero_unique_filtre, filtre.key) )){
        ...
        cat(crayon::red("\nNot all the Numero_unique_filtre_ADNe observed exist in the Filtre_ADNe table or DB.\n",
                        paste(unique(data$Extraits_ADNe$Numero_unique_filtre_ADNe[!data$Extraits_ADNe$Numero_unique_filtre_ADNe %in% c(Numero_unique_filtre, filtre.key)]), collapse = ", "),
    ...
    # Check with Extrait
      ...
      if(all(data$qPCR_inhibition_ADNe$Numero_unique_extrait_ADNe %in% c(Numero_unique_extrait, extrait.key) )){
        ...
        cat(crayon::red("\nNot all the Numero_unique_extrait_ADNe observed exist in the Extraits_ADNe table or DB.\n",
                        ...
                        paste(unique(data$qPCR_inhibition_ADNe$Numero_unique_extrait_ADNe[!data$qPCR_inhibition_ADNe$Numero_unique_extrait_ADNe %in%  c(Numero_unique_extrait, extrait.key)]), collapse = ", ")
                        , "\nare missing\n"))
      }
  ...
  if(c("qPCR_ADNe") %in%  names(data)  ){
    ...
    # Check with Courbe
      ...
      if(all(data$qPCR_ADNe$Courbe_ID %in% c(Courbe_ID, courbe.key) )){
        ...
        cat(crayon::red("\nNot all the Courbe_ID observed exist in the Courbe_etalonnage_ADNe table or DB.\n",


                        paste(unique(data$qPCR_ADNe$Courbe_ID[!data$qPCR_ADNe$Courbe_ID %in% c(Courbe_ID, courbe.key)]), collapse = ", ")
                        , "\nare missing\n"))
      }
  ...
  if(c("QNC_QPC_ADNe") %in%  names(data)  ){
    ...
    # Check with qPCR
      ...
      if(all(data$QNC_QPC_ADNe$ID_qPCR_ADNe %in%  c(qpcr_ID, qPCR.key) )){
        ...
        cat(crayon::red("\nNot all the ID_qPCR_ADNe observed exist in the qPCR_ADNe table or DB.\n",


                        paste(unique(data$QNC_QPC_ADNe$ID_qPCR_ADNe[!data$QNC_QPC_ADNe$ID_qPCR_ADNe %in% c(qpcr_ID, qPCR.key)]), collapse = ", ")
                        , "\nare missing\n"))
      }
    ...
    # Check with qPCR
      ...
      if(all(data$Sequencage_Sanger_ADNe$ID_qPCR_ADNe %in% c(qpcr_ID, qPCR.key))){
        ...
        cat(crayon::red("\nNot all the ID_qPCR_ADNe observed exist in the qPCR_ADNe table or DB.\n",
                        ...
                        paste(unique(data$Sequencage_Sanger_ADNe$ID_qPCR_ADNe[!data$Sequencage_Sanger_ADNe$ID_qPCR_ADNe %in% c(qpcr_ID, qPCR.key)]), collapse = ", ")
                        , "\nare missing\n"))
      }
  ...
  if(c("Librairies_ADNe") %in%  names(data)  ){
    ...
    # Check with Numero_unique_extrait_ADNe
      ...
      if(all(data$Librairies_ADNe$Numero_unique_librairie_ADNe %in% c(Purif_ID, purif.key))){
        ...
        cat(crayon::red("\nNot all the Numero_unique_librairie_ADNe observed exist in the Purification_librairies_ADNe table or DB.\n",


                        paste(unique(data$Librairies_ADNe$Numero_unique_librairie_ADNe[!data$Librairies_ADNe$Numero_unique_librairie_ADNe %in% c(Purif_ID, purif.key)]), collapse = ", ")
                        , "\nare missing\n"))
      }
  ...
  # Check with Numero_unique_librairie_SeqReady_ADNe
    ...
    if(all(data$Librairies_ADNe$Numero_unique_librairie_SeqReady_ADNe %in% c(Ext_ID, lib.key)  )){
      ...
      cat(crayon::red("\nNot all the Numero_unique_librairie_SeqReady_ADNe observed exist in the Analyses_externes_librairies_ADNe table or DB.\n",


                      paste(unique(data$Librairies_ADNe$Numero_unique_librairie_SeqReady_ADNe[!data$Librairies_ADNe$Numero_unique_librairie_SeqReady_ADNe %in% c(Ext_ID, lib.key)]), collapse = ", ")
                      , "\nare missing\n"))
    }
...
#  }
  ...
  # Librairies_SeqReady_ADNe : 2 checks
  ...
  if(c("Librairies_SeqReady_ADNe") %in%  names(data)  ){
    ...
    if(all(data$Librairies_SeqReady_ADNe$Numero_unique_librairie_ADNe %in% c(Purif_ID, purif.key))){
      cat(crayon::green("\nAll the Numero_unique_librairie_ADNe observed exist in the Purification_librairies_ADNe table or DB.\n"))
      ...
      cat(crayon::red("\nNot all the Numero_unique_librairie_ADNe observed exist in the Purification_librairies_ADNe table or DB.\n",
                      ...
                      paste(unique(data$Librairies_SeqReady_ADNe$Numero_unique_librairie_ADNe[!data$Librairies_SeqReady_ADNe$Numero_unique_librairie_ADNe %in% c(Purif_ID, purif.key)]), collapse = ", ")
                      , "\nare missing\n"))
    }
    ...
    if(all(data$Librairies_SeqReady_ADNe$Numero_unique_librairie_SeqReady_ADNe %in% c(Ext_ID, lib.key)  )){
      cat(crayon::green("\nAll the Numero_unique_librairie_SeqReady_ADNe observed exist in the Analyses_externes_librairies_ADNe table or DB.\n"))
      ...
      cat(crayon::red("\nNot all the Numero_unique_librairie_SeqReady_ADNe observed exist in the Analyses_externes_librairies_ADNe table or DB.\n",
                      ...
                      paste(unique(data$Librairies_SeqReady_ADNe$Numero_unique_librairie_SeqReady_ADNe[!data$Librairies_SeqReady_ADNe$Numero_unique_librairie_SeqReady_ADNe %in% c(Ext_ID, lib.key)]), collapse = ", ")
                      , "\nare missing\n"))
    }
